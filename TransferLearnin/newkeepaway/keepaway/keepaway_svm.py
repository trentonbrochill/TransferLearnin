...
def launch_player(player_type, index, options):
    ...
    command = [relative('./player_svm/keepaway_player')] + player_options
    #print command
    #print " ".join(command)
    Popen(command)
...
def parse_options(args = None, **defaults):
    ...
    parser.add_option(
        '--synch-mode', action = 'store_true', default = True,
    ...
    # added file for SA logging 
    parser.add_option( 
        '--SAlogFile', default = 'testSAlog.data',
        help = "Stores State-Action pairs for keyboard inputs")
