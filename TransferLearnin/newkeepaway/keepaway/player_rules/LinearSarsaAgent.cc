...
// Uses rules defined in Rules.h to seed a reinforcement learner using Value Bonus method
  ...
  SMDPAgent( numFeatures, numActions )
{
  ...
  B = 10; // reward bonus
  C = 100; // initial episodes where learner must use rules
  episode = 0;
...
int LinearSarsaAgent::startEpisode( double state[] )
{
  ...
  lastAction = LinearSarsaAgent::useRules(state);
  ruleAction = true;
...
int LinearSarsaAgent::step( double reward, double state[] )
{
  ...
  if (episode > C){
      ...
      if (lastAction == useRules(state)) ruleAction = true;
      else ruleAction = false;


  }
  else {
      episode++;
      lastAction = useRules(state);
      ruleAction = true;
  }
