  ...
  SMDPAgent( numFeatures, numActions )
{
  ...
  if((model=svm_load_model("best_mix_sub.libsvm.model"))==0)
  {
...
int LinearSarsaAgent::classifierAction(double state[]){
    ...
    svm_node state_node[14];


    for(int i = 0; i < 13; i++){
        float scaled_val;
        if(i < 5) scaled_val = state[i] / 25;
        else if(i < 9) scaled_val = state[i] / 15;
        else if(i < 11) scaled_val = state[i] / 30;
        else scaled_val = state[i] / 180;

        state_node[i].index = i + 1;
        state_node[i].value = scaled_val;
    }
